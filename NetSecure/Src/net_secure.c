...
int net_TLSConnect(NetTransportContext *ctx, SocketType sockType, const char* host, uint16_t port, uint32_t timeoutMs) {
    ...
    NETS_MSG("DNS lookup result: %s => %d.%d.%d.%d\r\n", host, targetIp[0], targetIp[1], targetIp[2], targetIp[3]);
...
int net_TLSConnectIp(NetTransportContext *ctx, SocketType sockType, uint8_t targetIp[4], uint16_t port, uint32_t timeoutMs) {
...
void net_TLSSetHandleClientConnectionCallback(NetHandleClientConnectionCallback callback) {
    ...
    net_SetHandleClientConnectionCallback(net_TLSNetHandleClientConnectionCallbackWrapper);
}
...
int net_TLSStartServerConnection(NetTransportContext *ctx, SocketType sockType, uint16_t port) {
    ...
    int rc = wolfSSL_CTX_use_certificate_buffer(ctx->sslCtx, DEVICE_CERT, DEVICE_CERT_SIZE, WOLFSSL_FILETYPE_ASN1);
    if (rc != WOLFSSL_SUCCESS) {
    ...
    rc = wolfSSL_CTX_use_PrivateKey_buffer(ctx->sslCtx, PRIVATE_KEY, PRIVATE_KEY_SIZE, WOLFSSL_FILETYPE_ASN1);
    if (rc != WOLFSSL_SUCCESS) {
...
int net_TLSSend(NetTransportContext *ctx, const char* buffer, const uint32_t bufferSz, uint32_t timeoutMs) {
...
int net_TLSReceive(NetTransportContext *ctx, char* buffer, const uint32_t bufferSz, uint32_t timeoutMs) {
    NETS_ENTER("net_TLSReceive");
    NETS_MSG("Request arrived to receive %lu byte of data\r\n", bufferSz);
    ...
    uint32_t totalReadSz = 0;
    uint32_t remainingRead = bufferSz;
    char *bufferPtr = buffer;
    do {
        int rc = wolfSSL_read(ctx->ssl, (char*) bufferPtr, remainingRead);
        if (rc < 0) {
            NETS_MSG("wolfSSL_read failed %d\r\n", rc);
            NETS_EXIT("net_TLSReceive", rc, 1);
            return rc;
        ...
        if (rc == 0) {
            NETS_MSG("wolfSSL_read - there's no more data to read\r\n");
            NETS_EXIT("net_TLSReceive", 0, 0);
            return totalReadSz;
        }
        totalReadSz += rc;
