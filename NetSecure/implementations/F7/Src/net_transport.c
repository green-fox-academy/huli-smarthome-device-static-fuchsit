...
int net_Connect(NetTransportContext *ctx, SocketType sockType, const char* host, uint16_t port, uint32_t timeoutMs) {
    ...
    NET_MSG("DNS lookup result: %s => %d.%d.%d.%d\r\n", host, destIp[0], destIp[1], destIp[2], destIp[3]);
...
int net_ConnectIp(NetTransportContext *ctx, SocketType sockType, uint8_t targetIp[4], uint16_t port, uint32_t timeoutMs) {
    ...
    enum netconn_type protocol = sockType == SOCKET_UDP ? NETCONN_UDP : NETCONN_TCP;
...
int net_StartServerConnection(NetTransportContext *ctx, SocketType sockType, uint16_t port) {
    ...
    enum netconn_type protocol = sockType == SOCKET_UDP ? NETCONN_UDP : NETCONN_TCP;
    uint8_t isTcp = (protocol == NETCONN_TCP);
    ...
    while (1) {
        if (isTcp) {
            int cliRc = netconn_accept(ctx->connection.conn, &ctx->connection.clientConn);
            if (cliRc != ERR_OK) {
...
int net_Disconnect(NetTransportContext *ctx) {
    ...
    int rc;
    if (ctx->connection.conn->type == NETCONN_TCP) {
        rc = netconn_close(ctx->connection.conn);
        if (rc != ERR_OK) {
            NET_MSG("ERROR: netconn_close\r\n");
            NET_EXIT("net_Disconnect", rc, 1);
            return rc;
        }
    }
    else {
        rc = netconn_disconnect(ctx->connection.conn);
        if (rc != ERR_OK) {
